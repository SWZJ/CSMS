	...
	<div class="col-md-8 col-md-offset-2" style="padding: 100px 70px 0 70px;">
		<div class="panel panel-default" style="text-align:center; vertical-align:middel;">
			...
			<div class="panel-body">
				...
				if(user.isExist_email(user_email)){
					out.print("<script>alert(\"绑定邮箱失败！\\n邮箱 "+user_email+" 已被绑定！\");window.history.back(-1);</script>");return;
				}
				Sendemail email = new Sendemail();
				String to = user_email;
				String title = "神葳总局邮箱绑定";
				...
				String content ="<div style='width:640px; background:#fff; border:solid 1px #efefef; margin:0 auto; padding:0 0 0 0'>"+
					...
					"      <p style='margin:5px 0; padding:3px 0;color:#666; font-size:14px'>神葳总局邮箱绑定通知:</p>"+
					"      <p style='color:#666; font-size:14px'>您正在进行神葳总局用户邮箱绑定，请在24小时内点击下面链接完成邮箱验证： "+
					"      <a href='"+basePathPort+"SWZJ/user/email/validateEmail.jsp?_token=lyGZwiU0GA6BPKKRIX9f8DSzkNptSO17ftcyrUlV&amp;operation=bindEmail&amp;email="+to+"&amp;account="+user.getAccount()+"&amp;id="+user.getID()+"&amp;poke="+poke+"' target='_blank' style='display:block; margin-top:10px; color:#2980b9; line-height:24px; text-decoration:none;word-break:break-all; width:575px;'>"+
					""+basePath+"SWZJ/user/email/validateEmail.jsp?_token=lyGZwiU0GA6BPKKRIX9f8DSzkNptSO17ftcyrUlV&amp;operation=bindEmail&amp;email="+to+"&amp;account="+user.getAccount()+"&amp;id="+user.getID()+"&amp;poke="+poke+"</a>"+
					"      </p>"+
				...
				<div>
					<h1>验证邮件发送失败</h1><hr>
					<h4>邮箱不存在或遇到未知问题，请重新绑定或联系管理员</h4>
				</div>
